...
General settings are included in a `config.ini` file (default example provided), which can typically be specified at a project level. Station-specific settings are normally few enough to use command line arguments without being too cumbersome. Most settings may be specified either as command line arguments or parameters in the INI file, except of course the path to the INI file itself if not using the default one. In the case of settings specified by multiple input methods, the command line arguments will take precedence.
...
|    `pdftex_path`     | `common`  |                                 | Path to PDFLaTex application                                                                                                                            |
...
Raw data from the Aquarius OBS is in miniSEED file format, with one channel per file and maximum file size of 128 MB, using the STEIM2 data compression algorithm. This means that the amount of data in a particular file varies with sampling rate and compression efficiency.
